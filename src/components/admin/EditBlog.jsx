...
import { createBlog, updateBlog } from "@/data/blogs";
...
export default function EditBlog({ blog, id }) {
  const HOST = "http://localhost:8000";
  // console.log("Logo", categories);
  const [title, setBlogTitle] = useState(blog ? blog.title : "");
  const [description, setBlogContent] = useState(blog ? blog.description : "");
  const [tag, setCategorySlug] = useState(blog ? blog.tag : "");

  // const [categories, setCategories] = useState([]);
  const [imagePreview, setImagePreview] = useState(blog ? blog.image : "");
